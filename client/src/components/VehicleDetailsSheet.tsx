...
export default function VehicleDetailsSheet({ vehicle, onClose }: { vehicle: any; onClose: () => void }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-3xl p-5 max-h-[85vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
        <div className="sticky top-0 -mx-5 -mt-5 px-5 pt-4 pb-2 bg-white/95 backdrop-blur z-20 flex items-center justify-between rounded-t-3xl">
          <h3 className="text-base font-bold text-[#111111] truncate pr-3">{vehicle.brand} {vehicle.model}</h3>
          <button
            onClick={onClose}
            aria-label="Fechar"
            className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center go-btn-active shrink-0"
          >
        ...
        {showImg && (
          <div className="relative mt-2 mb-4 rounded-2xl bg-gradient-to-br from-white via-slate-50 to-slate-200 h-36 overflow-hidden flex items-center justify-center">
        ...
        <div className="flex items-center gap-3 mb-3">
          <BrandMark brand={vehicle.brand} iconType={vehicle.iconType} className="w-10 h-10 flex-shrink-0" />
